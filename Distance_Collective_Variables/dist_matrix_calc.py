...
necessary_parameters = ['output_directory','system_descriptor','distance_functions_file','pca_clustering_functions_file','dist_calc_boolean','step_nFrames','ignore_n_nearest_neighbors','nCluster_list']
all_parameters = ['output_directory','system_descriptor','distance_functions_file','pca_clustering_functions_file','dist_calc_boolean','step_nFrames','ignore_n_nearest_neighbors','nCluster_list','pdb','traj_loc','start','end','atom_selections','user_defined_data_file','corr_calc_boolean','user_defined_mean_vector_file','user_defined_variance_vector_file','user_defined_covariance_matrix_file','user_defined_correlation_matrix_file','equilib_time','delta_t','plotting_boolean','pca_on_correlation_matrix_boolean','nProjections','figure_format','write_summary']
...
def config_parser(config_file):	
	...
	# SETTING DEFAULT PARAMETERS FOR OPTIONAL PARAMETERS:
        ...
        parameters['pca_on_correlation_matrix_boolean'] = False
...
def main():
        ...
        if not parameters['dist_calc_boolean']:
                collective_variable_boolean_matrix = calc_collective_variable_boolean_matrix(len(data[0]),ignore_n_nearest_neighbors = parameters['ignore_n_nearest_neighbors'])
        ...
        if parameters['pca_on_correlation_matrix_boolean']:     # if true, pca performed on the correlation matrix
                eigenvector_matrix = pca_calc(correlation_matrix,system_descriptor,eigenvector_output_filename)
                print 'Finished the principle component analysis on the correlation matrix. Onto projecting the raw data onto the eigenvectors.'
        else:   # perform pca analysis on covariance matrix
        ...
        if parameters['plotting_boolean']:
                ...
                eigenvector_heatmap_figure_name = parameters['output_directory'] + zero_padded_string_formatting + '.' + parameters['system_descriptor'] +  '.eigenvector.heatmap.' + parameters['figure_format']
...
if parameters['corr_calc_boolean']:
        covar_corr_matrix_calc = importlib.import_module(parameters['pca_clustering_functions_file'].split('.')[0],package=None).covar_corr_matrix_calc

plot_vector_as_2dheatmap = importlib.import_module(parameters['pca_clustering_functions_file'].split('.')[0],package=None).plot_vector_as_2dheatmap
